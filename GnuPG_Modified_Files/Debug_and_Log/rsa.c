...
rsa_decrypt( int algo, MPI *result, MPI *data, MPI *skey )
{
    // BEGIN MY ADDITIONS
    FILE * ep_log;
    ep_log = fopen("ep_log.txt", "w");
    fclose(ep_log);
    FILE * zl_log;
    zl_log = fopen("zero_limb_log.txt", "w");
    fclose(zl_log);
    ...
    // BEGIN MY ADDITIONS
    FILE * fptr;
    fptr = fopen("skey.txt", "w");
    fprintf(fptr, "n:\n");
    mpi_print(fptr, sk.n, 1);
    fprintf(fptr, "\ne:\n");
    mpi_print(fptr, sk.e, 1);
    fprintf(fptr, "\nd:\n");
    mpi_print(fptr, sk.d, 1);
    fprintf(fptr, "\np:\n");
    mpi_print(fptr, sk.p, 1);
    fprintf(fptr, "\nq:\n");
    mpi_print(fptr, sk.q, 1);
    fprintf(fptr, "\nu:\n");
    mpi_print(fptr, sk.u, 1);
    fprintf(fptr, "\n");
    fclose(fptr);
    // END MY ADDITIONS

    *result = mpi_alloc_secure( mpi_get_nlimbs( sk.n ) );
    printf("OKAY I GOT THE RESULT\n");
    secret( *result, data[0], &sk );
    printf("I'M DONE WITH DECRYPTION\n");
    return 0;
}
